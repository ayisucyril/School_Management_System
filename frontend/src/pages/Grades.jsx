...
const Grades = () => {
  ...
  const [classSubjects, setClassSubjects] = useState([]); // subjects for selected student's class
  ...
  // When student changes, find their class and load its subjects
  const handleStudentChange = studentId => {
    setForm(p => ({ ...p, studentId, subject: '' }));
    if (!studentId) { setClassSubjects([]); return; }

    const student = students.find(s => s._id === studentId);
    if (!student?.classId) { setClassSubjects([]); return; }

    // classId may be populated object or just an ID string
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">{grades.length} records</p>
        </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="glass-card p-6">
            ...
            <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {/* Subject — dropdown if class has subjects, text input otherwise */}
              <div>
                <label className="block text-white/50 text-xs mb-1.5">
                  Subject *
                  {classSubjects.length > 0 && (
                    <span className="ml-2 text-primary/60">({classSubjects.length} available)</span>
                  )}
                </label>
                {classSubjects.length > 0 ? (
                  <select
                  ...
                  <input
                    value={form.subject}
                    onChange={e => setForm(p => ({ ...p, subject: e.target.value }))}
                    placeholder={form.studentId ? "No subjects set for this class — type manually" : "Select a student first"}
                    className="input-field" required
                  />
                )}
                {form.studentId && classSubjects.length === 0 && (
                  <p className="text-white/30 text-xs mt-1">
                    💡 Ask admin to add subjects to this student's class.
                  </p>
                )}
              </div>
